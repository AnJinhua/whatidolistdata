  ...
  return Comment.findById(id, (error, comment) => {
    ...
    if (voter === undefined || !voter) {
      ...
      comment.voters = comment.voters.filter(
        (element) => element.slug !== author
      );
    }
    comment.voters_dislikes = comment.voters_dislikes.filter(
      (voters_dislike) => voters_dislike.slug !== author
    );
    return comment.save((err) => {
      if (err) {
        return res.json({
...
exports.deleteComment = (req, res) => {
  ...
  return Comment.remove(
    {
      $or: [{ _id: new mongoose.Types.ObjectId(id) }, { parentId: id }],
    },
    (error) => {
      if (error) {
        return res.json({
