        ...
        if (err) {
          ...
          sessionPayment.save((err1) => {
            ...
            Videosession.findOne(
              { _id: video_session_id },
              (err2, videosession) => {
                if (err2) {
                  bind.status = 0;
                  bind.message =
                    "Oops! error occured while fetching video session by id";
                  bind.error = err2;
                  return res.json(bind);
                }
                ...
                videosession.save((err3) => {
                  if (err3) {
                    bind.status = 0;
                    bind.message =
                      "Oops! error occured while updating video session";
                    bind.error = err3;
                  } else {
                    bind.status = 1;
                    ...
                    let data = {
                      ...
                      duration: `${moment
                        .duration(Number(videoSessionDuration), "seconds")
                        .format("ss")} seconds`,
                      user_name,
                      image: userImage,
                    };
                    expert_invoice.render(data, (err4, result) => {
                      const mailOptions = {
                        from: "Donnie's List <no-reply@whatido.app>",
                        to: expertEmail,
  ...
  if (expertEmail) {
    ...
    User.findOne({ email: expertEmail }, (err, expertInfo) => {
      ...
      if (expertInfo) {
        if (
          expertInfo &&
          expertInfo.expertSessionAvailability === true &&
          expertInfo.videoSessionAvailability === true
        ) {
          bind.status = 1;
          ...
          bind.status = 0;
          bind.message =
            "Expert session is not available right now! Please try after some time";
        }
      } else {
        bind.status = 0;
