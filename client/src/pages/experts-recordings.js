...
class Recordings extends Component {
  ...
  componentDidMount() {
    ...
    this.props.getExpertRecordings({ expertEmail }).then(
      (response) => {
        ...
        console.log(
          '**** getExpertRecordings success  ****' +
            JSON.stringify(this.state.recordings)
        )
      },
      (err) =>
        err.response.json().then(({ errors }) => {
          console.log(
            '**** getExpertRecordings error ****' + JSON.stringify(errors)
          )
        })
    )
  }
  ...
  playRecordedAudio(archiveId) {
    this.props.playRecordedAudio({ archiveId }).then(
      (response) => {
        console.log(
          '**** playRecordedAudio success  ****' + response.archive_url
        )
      ...
      (err) =>
        err.response.json().then(({ errors }) => {
          console.log(
            '**** playRecordedAudio error ****' + JSON.stringify(errors)
          )
        })
    )
  }
  ...
  deleteRecordedAudio(index, archiveId, id) {
    ...
    this.props.deleteRecordedAudio({ archiveId, id }).then(
      (response) => {
        console.log(
          '**** deleteRecordedAudio success  ****' + JSON.stringify(response)
        )
      ...
      (err) =>
        err.response.json().then(({ errors }) => {
          console.log(
            '**** deleteRecordedAudio error ****' + JSON.stringify(errors)
          )
        })
    )
  }
          ...
          {this.state.recordings.map((recording, index) => (
            <tr key={`TR_${index}`}>
              ...
              <td>
                <button
                  type='button'
                  onClick={this.playRecordedAudio.bind(
                    this,
                    recording.archiveId
                  )}
                ...
                <button
                  type='button'
                  onClick={this.openRecordingDeleteModal.bind(this)}
                ...
                <Modal
                  bsSize='small'
                  show={this.state.showRecordingDeleteModal}
                  onHide={this.closeRecordingDeleteModal.bind(this)}
                >
                  ...
                  <Modal.Footer>
                    <Button
                      bsStyle='danger'
                      onClick={this.deleteRecordedAudio.bind(
                        this,
                        index,
                        recording.archiveId,
                        recording._id
                    ...
                    <Button onClick={this.closeRecordingDeleteModal.bind(this)}>
                      Close
                    </Button>
                  </Modal.Footer>
                </Modal>
  ...
  render() {
    return (
      <div className='recordings-wrapper min-height-full-page'>
            ...
            <div className='row'>
              <div className='expert-list-inner-wrap'>
                <div className='col-sm-12'>
                  {/* <Card header={'Recordings'} bsStyle='primary'>
                    {this.recordingsList()}
                  </Card> */}
