      ...
      return axios
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            this.props.cookies.set("user", response.data.user, {
              path: "/",
              secure: false,
              sameSite: "Lax",
            });
      ...
      <div>
        ...
        >
          ...
          {COUNTRY_CODES.map((code, index) => (
            <option value={code.code} key={`COUNTRY_KEY_${index}`}>
            ...
            {/*}
              <div className="row pad15">
                <div className="col-md-6">
                  ...
                  {this.state.profileImage && this.state.profileImage != null && this.state.profileImage != "" ? <div><img loading="lazy" height="120" width="160" src={`${Image_URL}`+this.state.profileImage} /></div> : ""}
                  {this.state.RelatedImages1.length > 0 ? <div className="inline-block brdr img_preview_main"> {this.state.RelatedImages1.map((file,id) => <div className='img_preview' key={id}> <img loading="lazy" src={file.preview} alt="Preview Not Available" width={160} height={90}/></div> )} </div> : null}
