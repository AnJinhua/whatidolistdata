...
import { finshSignUp } from "../../../actions/auth";
...
export default function FinishSignUp(props) {
  ...
  const handleSubmit = (event) => {
    ...
    dispatch(finshSignUp(newData, query.get("token"))).then((res) => {
      if (res == undefined) {
        setState({ ...state, errorMessage: " Error Account Unauthorized " });
      } else {
        if (
          res.data.error &&
          res.data.error !== null &&
